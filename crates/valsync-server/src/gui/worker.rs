...
pub(super) fn invite_code(cfg: &Config, kp: &Keypair) -> Result<String> {
    valsync_core::Invite::new(cfg.public_url(), &kp.public(), cfg.server.name.trim())
        .encode()
    ...
    let result = (|| -> Result<()> {
        ...
        rt.block_on(async move {
            ...
            let listener = tokio::net::TcpListener::bind(addr).await.with_context(|| {
                format!("cannot listen on {addr}; is another valsync-server running?")
            })?;
...
pub(super) fn public_ip() -> Result<Msg> {
    let client = reqwest::blocking::Client::builder()
        ...
        .user_agent(concat!("valsync-server/", env!("CARGO_PKG_VERSION")))
        .build()?;
    let text = client
        .get(IP_ECHO_SERVICE)
        .send()
