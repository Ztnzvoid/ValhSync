...
//! game folder (what is there) and the previous installed state (what ValSync
...
pub enum Action {
    ...
    Replace,
    /// Installed by ValSync earlier, no longer in the manifest: move to backup.
    Remove,
...
mod tests {
    ...
    fn replace_seed_remove_and_quarantine() {
        ...
        write(
            g,
            "BepInEx/_valsync_quarantine/20260101-000000/BepInEx/plugins/x.dll",
            b"q",
        );
        ...
        assert!(
            plan.items
                .iter()
                .all(|i| !i.path.contains("_valsync_quarantine"))
        );
