...
enum Cmd {
    ...
    /// Print what ValSync knows about this machine.
    Doctor,
}
...
fn cmd_join(ctx: &Context, code: &str, replace_key: bool) -> Result<()> {
    ...
    println!("Next: `valsync status` to preview, `valsync play` to sync and start the game.");
    Ok(())
}
...
fn cmd_servers(ctx: &Context) -> Result<()> {
    ...
    if book.servers.is_empty() {
        println!("No server yet. Import an invite code with `valsync join <code>`.");
        return Ok(());
    }
...
fn sync(ctx: &Context, prepared: &Prepared, yes: bool) -> Result<()> {
    ...
    if let Some(stamp) = &applied.backup_stamp {
        println!("Backup {stamp} kept; `valsync rollback` undoes this sync.");
    }
...
fn cmd_rollback(ctx: &Context, list: bool) -> Result<()> {
    ...
    println!("The next `valsync sync` will re-apply the server's pack.");
    Ok(())
}
