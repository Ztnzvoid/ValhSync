...
pub enum SyncError {
    #[error(
        "cannot reach the server at {url}: {reason}. Check your connection, or ask the admin whether valsync-server is running"
    )]
    ...
    #[error(
        "the server's signing key does not match the one pinned for \"{name}\" (pinned {pinned}, received {received}). Nothing was changed. Ask the admin for a fresh invite code and import it with `valsync join --replace-key`"
    )]
    KeyMismatch {
        name: String,
        pinned: String,
    ...
    #[error(
        "Valheim was not found. Tell ValSync where it is with `valsync game-root <folder>` (the folder containing valheim.exe)"
    )]
    GameNotFound,
    ...
    #[error("no server known yet; import an invite code with `valsync join <code>`")]
    NoServer,

    #[error("no server named {0:?}; run `valsync servers` to list them")]
    ...
    #[error(
        "the sync failed while applying changes ({reason}) AND restoring backup {stamp} failed too ({rollback_error}). Fix the cause, then run `valsync rollback`"
    )]
    RollbackFailed {
        reason: String,
        stamp: String,
    ...
    #[error(
        "first sync needs a confirmation; review the plan with `valsync status` and run again with --yes"
    )]
    NeedsConfirmation,
