...
//!
//! - config dir (`%APPDATA%\valsync`, `~/.config/valsync`): known servers,
//!   settings, installed state
//! - backups dir (`%LOCALAPPDATA%\valsync\backups`, `~/.local/share/valsync/backups`)
//!
...
impl AppPaths {
    ...
    pub fn discover() -> Result<Self> {
        ...
        let paths = Self {
            config_dir: base.config_dir().join("valsync"),
            backups_dir: base.data_local_dir().join("valsync").join("backups"),
        };
        paths.ensure()?;
        Ok(paths)
    }
...
pub fn copy_atomic(src: &Path, dst: &Path) -> Result<()> {
    ...
    let tmp = dst.with_extension("valsync-tmp");
